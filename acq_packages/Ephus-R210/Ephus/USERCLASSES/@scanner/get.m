% scanner/get - Query internal fields of the object.
%
% SYNTAX
%  PROPERTIES = get(aod) - Gets all the fields in a scanner object.
%  PROPERTY = get(aod, NAME) - Gets the value of the NAME field in a scanner object.
%  PROPERTIES = get(aod, NAME, ...) - Gets the value of each named field in a scanner object.
...
if isempty(varargin)
    varargout{1} = scannerObjectsGlobal(this.ptr);
    return;
end
...
for i = 1 : length(recognized)
    varargout{i} = scannerObjectsGlobal(this.ptr).(recognized{i});
end
