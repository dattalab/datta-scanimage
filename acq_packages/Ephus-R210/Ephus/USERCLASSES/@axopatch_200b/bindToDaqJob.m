...
%  VI081308A - Don't bind the data listeners twice. Can cause intermittent errors (the most annoying kind). -- Vijay Iyer 8/13/08
%  TO060810E - Disable the vHold listener function, nobody is currently using this information and the listener is responsible for problems when stopping (it can cause the acquisition to appear to keep running). -- Tim O'Connor 6/8/10
%
...
if ~isChannel(job, gainName)
    ...
    %bindDataListener(job, gainName, {@axopatch200b_gainSamplesAcquiredFcn, this}, [name '-gainTelegraph']);%TO110607A, VI081308A
end
...
if ~isChannel(job, modeName)
    ...
    %bindDataListener(job, modeName, {@axopatch200b_modeSamplesAcquiredFcn, this}, [name '-modeTelegraph']);%TO110607A, VI081308A
end
...
if ~isChannel(job, v_holdName)
    ...
    %bindDataListener(job, v_holdName, {@axopatch200b_vholdSamplesAcquiredFcn, this}, [name '-vholdTelegraph']);%TO110607A, VI081308A
end
...
% bindDataListener(job, v_holdName, {@axopatch200b_vholdSamplesAcquiredFcn, this}, [name '_vHoldListener']);%TO101707F %TO060810E
