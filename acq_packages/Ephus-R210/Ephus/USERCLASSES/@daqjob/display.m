...
%  VI081308A - Added readErrorMode field -- Vijay Iyer 8/13/08
%  VI102308A - Hanlde correct display of triggerDestinations & triggerDestination properties (latter being virtual) -- Vijay Iyer 10/23/08
%
% Created
...
%%%VI102308A%%%%%%%%%%%%%%%
if isempty(daqjobGlobalStructure(this.ptr).triggerDestinations)
    fprintf(1,'\ttriggerDestinations: NONE\n');
    fprintf(1,'\ttriggerDestination: NONE\n');
else
    triggerDestinations = daqjobGlobalStructure(this.ptr).triggerDestinations;
    destStr = '{';
    for i=1:length(triggerDestinations)
        destStr = [destStr '''' triggerDestinations{i} ''' '];
    end
    destStr = [destStr(1:end-1) '}'];
    fprintf(1, '\ttriggerDestinations: %s\n', destStr);
    fprintf(1, '\ttriggerDestination: ''%s''\n', getTriggerDestination(this)); 
end
%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fprintf(1, '\treadErrorMode: %s\n', daqjobGlobalStructure(this.ptr).readErrorMode);%VI081308A
