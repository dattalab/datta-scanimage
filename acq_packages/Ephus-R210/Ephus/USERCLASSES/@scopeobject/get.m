% SCOPEOBJECT/GET - Query internal fields of a SCOPEOBJECT object.
%
% SYNTAX
%  PROPERTIES = get(SCOPEOBJECT) - Gets all the fields in a SCOPEOBJECT object.
%  PROPERTY = get(SCOPEOBJECT, NAME) - Gets the value of the NAME field in a SCOPEOBJECT object.
%  PROPERTIES = get(SCOPEOBJECT, NAME, ...) - Gets the value of each named field in a SCOPEOBJECT object.
...
if isempty(varargin)
    varargout{1} = scopeObjects(this.ptr);
    return;
end
...
for i = 1 : length(recognized)
    varargout{i} = scopeObjects(this.ptr).(recognized{i});
end
