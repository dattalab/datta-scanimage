...
%TO032510B - As per Ben, in Gordon's lab, this doesn't need to give the value per division, just the units.
%Axis labels.
...
%     'FontSize', scopeObjects(this.ptr).fontSize, 'Color', scopeObjects(this.ptr).foregroundColor, 'FontWeight', scopeObjects(this.ptr).fontWeight);
set(get(scopeObjects(this.ptr).axes, 'XLabel'), 'String', scopeObjects(this.ptr).xUnitsString, ...
    'FontSize', scopeObjects(this.ptr).fontSize, 'Color', scopeObjects(this.ptr).foregroundColor, 'FontWeight', scopeObjects(this.ptr).fontWeight);
set(get(scopeObjects(this.ptr).axes, 'YLabel'), 'String', scopeObjects(this.ptr).yUnitsString, ...
    'FontSize', scopeObjects(this.ptr).fontSize, 'Color', scopeObjects(this.ptr).foregroundColor, 'FontWeight', scopeObjects(this.ptr).fontWeight);
...
updateScaling(this); %JL081807B comment out updatescaling %TO031010J - We want to do our own scaling on the X-axis (to get a fixed width display, when the trace length is known a priori. GS031010_REMOTE_DEBUG_SESSION
