...
%  BSTO060910C: Make sure guis wind up on screen after loading a stored position. -- Ben Suter/Tim O'Connor 6/9/10
%
                ...
                if isfield(progmanagerGuisConfig, programs{i})
                    ...
                    for j = 1 : length(guiNames)
                        if ~ismember(guiNames{j}, runningGuis)
                            ...
                            %movegui(progmanagerglobal.programs.(programs{i}).guinames.(guiNames{j}).fighandle);%BSTO060910C - Scratch that, it moves things it shouldn't move, which could be annoying.
...
%     progmanagerGuisConfig.(programs{i}).guis.(guiNames{j}).visible);
            ...
            fprintf(2, 'Warning: Failed to properly load GUI metadata for program ''%s'': %s\n', programs{i}, lasterr);
        end
