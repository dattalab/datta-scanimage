...
%  TO031010D: Remove stale files/data. GS031010_REMOTE_DEBUG_SESSION -- Tim O'Connor 3/10/10
%
...
%TO031010D - Remove stale files/data. GS031010_REMOTE_DEBUG_SESSION
if ~noOverwrite
    files = dir(savePath);
    for i = 1 : length(files)
        if strcmpi(files(i).name, 'guiMetaDataInfo.mat') || endsWithIgnoreCase(files(i).name, '.settings')
            delete(fullfile(savePath, files(i).name));
        end
    end
end
