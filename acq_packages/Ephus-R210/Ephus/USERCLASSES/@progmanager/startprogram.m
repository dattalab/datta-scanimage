...
%     TO031010M: Sorta fixed the online documentation, since Vijay completely rejiggered the wiki (broke the direct URLs to the gui docs). GS031010_REMOTE_DEBUG_SESSION -- Tim O'Connor 3/10/10
%     TO031610E: Improved the online help. -- Tim O'Connor 3/16/10
%     BS042010A - Added two menu items for creating new user folder and cloning settings
%     TO052210B: Temporarily removed BS042010A until we can do some more testing/debugging. -- Tim O'Connor 5/22/10
%     BS060910A: re-enable only "Copy settings" menu item (the "New user folder" command is now a stand-alone m-file function)
%     BS060910B: remove the ProgramManagerGUI item from the program manager sub-menu
%     BS060910C: fix separators in main and program manager sub-gui
%
...
%     addmenu(main_fig_handle,1,labels,cbs,tags,sep,accel);
    ...
    %BS042010A - Added two menu items for creating new user folder and cloning settings
    %TO052210B - Temporarily removed BS042010A until we can do some more testing/debugging. -- Tim O'Connor 5/22/10
    %BS060910A - Re-adding only Copy Settings command
    labels = {'File&', 'New', 'Open...', 'Save', 'Save As...', 'Copy settings', 'Program Manager', 'About', 'Exit', 'Exit All'};
...
%     cbs = {'', 'disp(''Open...'')', 'disp(''Save'')','disp(''Save As...'')', 'disp(''Program Manager'')', 'disp(''Exit'')', 'disp(''Exit All'')'};
    cbs = {'', {@genericNewData, main_fig_handle}, {@genericOpenData, main_fig_handle}, {@genericSaveProgramData, main_fig_handle}, ...
        {@genericSaveProgramDataAs, main_fig_handle}, @promptCloneSettings, '', '', 'closeprogram(progmanager, gcbf)', @closeAllPrograms}; %BS042010A
    %cbs = {'', {@genericNewData, main_fig_handle}, {@genericOpenData, main_fig_handle}, {@genericSaveProgramData, main_fig_handle}, ...
    %    {@genericSaveProgramDataAs, main_fig_handle}, '', '', 'closeprogram(progmanager, gcbf)', @closeAllPrograms};
    tags = {'fileMenu', 'newMenuItem', 'openMenuItem', 'closeMenuItem', 'saveMenuItem', 'saveAsMenuItem', 'cloneSettings', 'progmanagerSubMenuMenuItem', 'aboutSubMenu', 'exitMenuItem', 'exitAllMenuItem'}; %BS042010A
    %tags = {'fileMenu', 'newMenuItem', 'openMenuItem', 'saveMenuItem', 'saveAsMenuItem', 'progmanagerSubMenuMenuItem', 'aboutSubMenu', 'exitMenuItem', 'exitAllMenuItem'};
    sep = {'Off', 'Off','Off','Off','Off', 'On', 'Off', 'Off', 'On', 'Off', 'Off'}; %BS060910C -- fix separators
    ...
    %TO111805A, TO062306F
    %BS060910B -- get rid of programmanagergui sub-menu item
    submenuLabels = {'Save Configuration', 'Load Configuration', 'Save Configuration Set', 'Load Configuration Set'};
    submenuCallbacks = {'saveConfig(progmanager, gcbo)', 'loadConfig(progmanager, gcbo)', 'saveConfigurations(progmanager)', 'loadConfigurations(progmanager)'};
    submenuTags = {'saveConfigurationMenuItem', 'loadConfigurationMenuItem', 'saveConfigurationSetMenuItem', 'loadConfigurationSetMenuItem'};
    submenuSep = {'Off', 'Off', 'On', 'Off'};
    submenuAccel = {'', '', '', ''};
%     progmanagerMenu = addmenu(main_fig_handle, 1, 'ProgramManagerGUI', 'progmanagerdisp(progmanager)', 'programManagerGuiMenuItem', 'Off', '');
    ...
    %set(progmanagerMenu, 'Parent', menuHandles(6));%TO090506I %BS042010C %TO052210B
    %set(progmanagerMenu, 'Parent', menuHandles(8));%TO090506I %BS042010C %TO052210B %BS060910A
    set(progmanagerMenu, 'Parent', menuHandles(7));%BS060910A
    ...
    submenuLabels = {'Version', 'Gui Specific Online Help', 'Online Help Home'};
    ...
    %TO031010M - Sorta fixed the online documentation, since Vijay completely rejiggered the wiki (broke the direct URLs to the gui docs). GS031010_REMOTE_DEBUG_SESSION -- Tim O'Connor 3/10/10
    %submenuCallbacks = {'displayVersionDialog(progmanager, gcbf)', 'web https://openwiki.janelia.org/wiki/display/ephus/Ephus+Documentation -new'};
    submenuCallbacks = {'displayVersionDialog(progmanager, gcbf)', sprintf('web http://openwiki.janelia.org/wiki/display/ephus/%s/ -new', program_name), ...
        'web http://openwiki.janelia.org/wiki/display/ephus/Ephus+Documentation -new'};%TO031610E
    submenuTags = {'versionMenuItem', 'onlineHelpMenuItem', 'onlineHelpHomeMenuItem'};
    submenuSep = {'Off', 'Off', 'Off'};
    submenuAccel = {'', '', ''};
    aboutMenu = addUiMenuItems(main_fig_handle, 1, submenuLabels, submenuCallbacks, submenuTags, submenuSep, submenuAccel);%TO071807B
    %set(aboutMenu, 'Parent', menuHandles(7));%TO090506I %BS042010D %TO052210B
    %set(aboutMenu, 'Parent', menuHandles(9));%TO090506I %BS042010D %TO052210B %BS060910A
    set(aboutMenu, 'Parent', menuHandles(8));%BS060910A
    
    
    %TO031610E
    if str2num(matlabVersionString(1:3)) >= 7
        helpMenuHandle = uimenu(main_fig_handle, 'Label', 'Help');
    else
        helpMenuHandle = uimenu(main_fig_handle, 'Label', 'Help&');
    end
    uimenu(main_fig_handle, 'Label', 'Version', 'Callback', 'displayVersionDialog(progmanager, gcbf)', 'Tag', 'versionMenuItem', 'Parent', helpMenuHandle);
    uimenu(main_fig_handle, 'Label', 'Gui Specific Online Help', 'Callback', ...
        sprintf('web http://openwiki.janelia.org/wiki/display/ephus/%s/ -new', program_name), 'Tag', 'Gui Specific Online Help', 'Parent', helpMenuHandle);
    uimenu(main_fig_handle, 'Label', 'Online Help Home', 'Callback', 'web http://openwiki.janelia.org/wiki/display/ephus/Ephus+Documentation -new', ...
        'Tag', 'helpHomeMenuItem', 'Parent', helpMenuHandle);
...
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%BS042010A - Added two menu items for creating new user folder and cloning settings
%BS060910A - Removed the "new user folder" function
function promptCloneSettings(varargin)
    % Prompt the user to select a source folder and a target folder (which must be empty)
