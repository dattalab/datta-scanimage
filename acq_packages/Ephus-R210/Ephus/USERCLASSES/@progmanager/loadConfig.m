...
try
    ...
    if ~isempty(loadedSettings)
        ...
        for j = 1 : length(guiNames)
            ...
            %movegui(progmanagerglobal.programs.(program_name).guinames.(guiNames{j}).fighandle);%BSTO060910C - Scratch that, it moves things it shouldn't move, which could be annoying.
