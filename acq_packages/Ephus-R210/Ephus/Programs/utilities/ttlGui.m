function varargout = ttlGui(varargin)
% TTLGUI M-file for ttlGui.fig
%      TTLGUI, by itself, creates a new TTLGUI or raises the existing
%      singleton*.
%
%      H = TTLGUI returns the handle to a new TTLGUI or the handle to
%      the existing singleton*.
%
%      TTLGUI('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in TTLGUI.M with the given input arguments.
%
%      TTLGUI('Property','Value',...) creates a new TTLGUI or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before ttlGui_OpeningFunction gets called.  An
%      unrecognized property ttlName or invalid value makes property application
%      stop.  All inputs are passed to ttlGui_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help ttlGui
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @ttlGui_OpeningFcn, ...
                   'gui_OutputFcn',  @ttlGui_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% ------------------------------------------------------------------
% --- Executes just before ttlGui is made visible.
function ttlGui_OpeningFcn(hObject, eventdata, handles, varargin)
% Choose default command line output for ttlGui
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes ttlGui wait for user response (see UIRESUME)
% uiwait(handles.figure1);
return;

% ------------------------------------------------------------------
% --- Outputs from this function are returned to the command line.
function varargout = ttlGui_OutputFcn(hObject, eventdata, handles)
