% shared_saveTrace - Save the previously acquired trace to a given file.
%
% SYNTAX
%  shared_saveTrace(hObject)
%  shared_saveTrace(hObject, filename)
%   filename - The file in which to save the trace.
%
% USAGE
%
% NOTES
%  Adapted from ephys_saveTrace. See TO101707F.
%
% CHANGES
...
function shared_saveTrace(hObject, varargin)
% getStackTraceString
% fprintf(1, '%s - ''%s''_saveTrace\n', datestr(now), getProgramName(progmanager, hObject));
...
%     end
%     if i == length(buffers)
% % fprintf(1, '%s - ''%s''_saveTrace: NO_DATA_1\n', datestr(now), getProgramName(progmanager, hObject));
%         return;
%     end
% end
% 
...
data = shared_getData(hObject);%TO123005E
