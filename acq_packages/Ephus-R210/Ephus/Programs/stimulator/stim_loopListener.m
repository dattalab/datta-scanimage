% stim_loopListener - Callback for handling loop events.
%
% SYNTAX
%  stim_loopListener(hObject, eventdata)
%
% USAGE
%
% NOTES
%  This is a copy & paste of ephys_loopListener.m with some editting, as needed.
%
% CHANGES
...
function stim_loopListener(hObject, eventdata)
global loopManagers;
    ...
    case 'loopstart'
        ...
        if getLocal(progmanager, hObject, 'startButton')
            if ~getLocal(progmanager, hObject, 'externalTrigger')
                error('stimulator must be set to external triggering for use in a loop.');
                stop(loopManager);
            end
...
%             %TO121505I
%             fprintf(1, '%s - stim_loopListener Warning: Loop iteration tried to execute before an acquisition has completed.\n%s\n', datestr(now), getStackTraceString);
%             return;
%         end
        if looping
