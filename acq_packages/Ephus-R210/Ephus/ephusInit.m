...
amp(1).scaledOutputBoardID = [3];        % Number identifying DAQmx board (e.g. 1 for 'Dev1') on which 'scaled output' signal from amplifier channel is input to Ephus
...
amp(1).vComBoardID = [3];                % Number identifying DAQmx board (e.g. 2 for 'Dev2') on which command signal from Ephus is output to amplifier channel   
...
amp(1).amplifierChannelID = [1];         %(MULTICLAMP ONLY) Identifies which amplifier 'channel' on the amplifier device, e.g. 1 or 2 on Multiclamp amplifiers.
amp(1).serialNumber = [00831864];               %(MULTICLAMP 700B ONLY) Identifies serial number of amplifier device on which this channel is located
...
acqChannelNames = {'chan4','chan5','chan6','chan7'};                   % Cell array of descriptive names for analog input acquisition channels to configure, e.g. {'Lick Sensor' 'Nose Poke Sensor'}, 
acqBoardIDs = [3,3,3,3];                       % A single number (e.g. 1 for 'Dev1') specifying DAQmx board for /all/ named acquisition channels; or, an array of numbers of length equal to 'acqChannelNames' (e.g. [1 1 1 2 2] indicating 'Dev1' for first 3 channels, 'Dev2' for last 2 channels) identifying DAQmx board on which each of the named acquisition channels appears. 
...
stimChannelNames = {'olfactoTrigPulse', 'olfactoStatePulse', 'sniffPulse'};                  % Cell array of descriptive names for analog output stimulus channels to configure, e.g. {'Whisker Stimulator' 'Position Encoder'}, 
stimBoardIDs = [5, 5, 3];                      % A single number (e.g. 1 for 'Dev1') specifying DAQmx board for /all/ named analog stimulus channels; or, an array of numbers of length equal to 'stimChannelNames' (e.g. [1 1 1 2 2] indicating 'Dev1' for first 3 channels, 'Dev2' for last 2 channels) identifying DAQmx board on which each of the named analog stimulus channels appears. 
stimChannelIDs = [0, 1, 1];                    % Array of numbers, of length equal to 'stimChannelNames', identifying DAQmx channel number (e.g. 1 for AI1) for each of the named analog stimulus channels (e.g. [0 1 2 0 1] indicating AI0-2 for first 3 channels  and AI0-1 for last 2 channels, for case of multiple boards). 
