...
%   properties on the LSC i) before a two-step move and ii) after the fast
%   stage of a two-step move. The original values of these LSC properties
%   are cached, and restored after the two-step move. Strictly speaking,
%   despite this restoration of properties, the state of the LSC after a
%   two-step move is not guaranteed to be identical to its state before a
%   two-step move, since how the LSC handles property setting is its own
%   business. For example, some LSC properties may be indexed by
%   'resolutionMode', in which case it is possible (albeit a bit
...
%   their specific LSC.
%
%   One-step moves use the prevailing settings on the LSC, whatever they
%   may be. If two-step moves are enabled, the LSC is initialized to match
%   the "slow" move configuration, so that one-step moves in this case are
%   slow. (Recall that when twoStepEnable is on, moves below a certain
%   distance threshold are done in one step.)
    ...
    properties (Constant)
        twoStepPropNames = {'moveMode' 'resolutionMode' 'velocity'};
...
%% CTOR/DTOR    
    ...
    methods
        ...
        function obj = StageController(lscObj,varargin)
            ...
            % make sure hLSC is initialized to "slow" settings if
            % twoStepEnable is on. when twoStepEnable is on, one-step moves
            % use the default LSC settings, so we want those settings to be
            % slow.
            if obj.twoStepEnable
                slowPropVals = obj.twoStepSlowPropVals;
                fnames = fieldnames(slowPropVals);
                for c = 1:numel(fnames)
                    % Don't bother using lscSet here; if these sets fail,
                    % the LSC was never even initted
                    obj.hLSC.(fnames{c}) = slowPropVals.(fnames{c}); % throws
                end
            end
...
%% DEVELOPER METHODS    
    ...
    methods (Hidden)
        ...
        function startOneStepMove(obj,absTargetPos)
            ...
            obj.lscFcn('moveStartAbsolute',absTargetPos,@obj.oneStepCbk);
        end
        ...
        function moveType = determineMoveType(obj,absTargetPosn)
            if obj.twoStepEnable
                if isempty(obj.twoStepDistanceThreshold)
                    ...
                    distance = norm(absTargetPosn(activeDims) - absPos(activeDims));
