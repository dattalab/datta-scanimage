    ...
    methods
        ...
        function initialize(obj)
            ...
            try
                ...
                fprintf(2,'*****************\n');
                fprintf(2,'Error occurred in intializing property ''%s'' of ''%s'' object''s Controller\n',propNames{i},class(obj.hModel));
                ME.rethrow();
            end
        end
            ...
            if ischar(userData) %Direct uicontrol
                ...
                switch ctlStyle 
                    ...
                    case 'popupmenu'
                        ...
                        propVal = options{get(src,'Value')}; %Encode as string of the one-and-only selected option
                    case 'uimenu'
                ...
                switch ctlStyle
                    case 'popupmenu'
                        opts = obj.hModel.getPropOptions(propName);
                        if ~isempty(opts) && isnumeric(opts) ... % numeric popup options provided by model
                            || isempty(opts) && zlclIsPropNumericClass(obj.hModel.mdlPropAttributes,propName) % popup options provided by GUIDE; model says prop has numeric class
