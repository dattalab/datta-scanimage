classdef AndorCamera < most.APIWrapper & most.PDEPProp
    ...
    %% TODO
    % High Priority (2011-05-16)
    %   TODO: Either disallow creation of multiple handles per device, or use copyable interface (avaialble in 2011a)
    %   
    % Older Lists
    ...
    %
    %   TODO: Make pixel saturation detection built into this class -- Vijay Iyer 8/9/10
    %   TODO: Investigate callback registration handling when deleting & reconstructing camera objects -- Vijay Iyer 8/9/10
                  
    
    %% USER PROPERTIES (ADDED BY CLASS)
    ...
    properties (Access=protected, Hidden)
        apiFuncNargoutMap; % Map keyed by driver function names, containing number of output arguments for each function, not including 'status'
        apiResponseCodeMap; 
        ...
        outputAmplifierMap;
        acquisitionModeMap = containers.Map({'single scan' 'accumulate' 'kinetics' 'fast kinetics' 'run till abort'},num2cell(1:5));
        readModeMap = containers.Map({'full vertical binning' 'multi track' 'random track' 'single track' 'image'},num2cell(0:4));
        triggerModeMap = containers.Map({'internal' 'external' 'external start' 'external exposure' 'external fvb em' 'software trigger'},num2cell([0 1 6 7 9 10]));
        indexMap = containers.Map({'bitDepth' 'preAmpGain' 'VSSpeed'},{'ADChannel' 'preAmpGainIdx' 'VSSpeedIdx'});
        ...
        invalidUntilInitProps = containers.Map( {'complexImage' 'cropMode'  'fastKinetics'  'image' 'isolatedCropMode'  'multiTrack'    'randomTracks'  'ringExposureTimes' 'singleTrack'}, ...
                                                {1              1           1               1       1                   1               1               1                   1});                                                                              
    ...
    %% ABSTRACT PROPERTY REALIZATION (most.APIWrapper)
     ...
     properties (Constant, Hidden)
        apiPrettyName = 'Andor SDK';  %A unique descriptive string of the API being wrapped
        apiCompactName = 'AndorSDK'; %A unique, compact string of the API being wrapped (must not contain spaces)
        apiSupportedVersionNames = {'2.90'}; %A cell array of shorthand names (strings) for API versions supported by this wrapper class
        
        apiDLLNames = 'atmcd32d'; %version-indexed. The name of the DLL sans the '.dll' extension.
        apiHeaderFilenames = 'atmcd32d.h'; %version-indexed. The name of the header file (with the '.h' extension - OR a .m or .p extension).
    ...
    properties (SetAccess=protected, Hidden)
        ...
        apiResponseCodeSuccess = 20002; %<NUMERIC> If specified, the first output argument of API 'standard' functions is taken to be a response code, with the specified response value(s) indicating call was successful.
        apiResponseCodeProcessor = '^#define (DRV\w*)\s*(\d*)'; %<One of {'none', 'apiResponseCodeMapHookFcn','apiResponseCodeHookFcn', <responseCodeMap regular expression>} - Default = 'none'>
        
        %API 'custom' cached data variables
        apiCachedDataVarMap = containers.Map({'cameraTypeMap'},{'zprvExtractCameraTypeMap'}); %A Map whose keys (strings) specify custom class-specific data variables to store to API Data file, and whose values (strings) specify method names used to extract each of the 'apiCachedDataVars'. If same name is used for more than one variable, method is only invoked once.
        ...
        apiHeaderRootPath = 'c:\program files\andor solis\drivers'; %NOT version-indexed. Either 'class', 'package', or <real path>. 'class' and 'package' indicate class or package private path, respectively.
    ...
    methods
        ...
        function obj = AndorCamera(cameraSerialNumber)
            ...
            %Store some API data vars as class props
            obj.apiFuncNargoutMap = obj.accessAPIDataVar('apiFuncNargoutMap');
            obj.apiResponseCodeMap = obj.accessAPIDataVar('apiResponseCodeMap');

            %Determine # of cameras & obtain handle to this particular camera
            numCameras = obj.apiCall('GetAvailableCameras',0);
            %obj.apiProcessErrorResponseCode(status,'GetAvailableCameras');
            ...
            if numCameras > 1
                for i=1:numCameras
                    ...
                    cameraHandle = obj.apiCall('GetCameraHandle', i-1, 0);
                    %                     [status,cameraHandle] = obj.apiCallRaw('GetCameraHandle', i-1, 0);
                    %                     obj.apiProcessErrorResponseCode(status,'GetCameraHandle');
                    
                    obj.apiCall('SetCurrentCamera',cameraHandle)
                    %                     status = obj.apiCallRaw('SetCurrentCamera',cameraHandle);
                    %                     obj.apiProcessErrorResponseCode(status,'SetCurrentCamera');
                    
                    %Initialize camera (needed before getting serial number)
                    obj.apiCall('Initialize',obj.driverPath);
                    %                     status = obj.apiCallRaw('Initialize',obj.driverPath);
                    %                     obj.apiProcessErrorResponseCode(status,'Initialize');
                    
                    % get the serial number for the current handle
                    currentSerialNumber = obj.apiCall('GetCameraSerialNumber',0);
                    %                     [status, currentSerialNumber] = obj.apiCallRaw('GetCameraSerialNumber',0);
                    %                     obj.apiProcessErrorResponseCode(status,'GetCameraSerialNumber');
                ...
                obj.cameraHandle = obj.apiCall('GetCameraHandle', 0, 0);
                %                 [status,obj.cameraHandle] = obj.apiCallRaw('GetCameraHandle', 0, 0);
                %                 obj.apiProcessErrorResponseCode(status,'GetCameraHandle');
            end
            
            %Initialize the Andor SDK
            obj.apiCall('Initialize', obj.apiDLLPaths(obj.apiCurrentVersion)); %This takes camera handle and its control is reserved to this object (i.e. can't use in Solis)
            
            %Store the cameraSerialNumber to the class instance, rather than using a pseudo-dependent property
            obj.cameraSerialNumber = obj.apiCall('GetCameraSerialNumber',0);
            %[status,obj.cameraSerialNumber] = obj.apiCallRaw('GetCameraSerialNumber',0);
        ...
        function delete(obj)
            ...
            if ~isempty(obj.cameraHandle)
                ...
                startTemperature = obj.apiCallFiltered('GetTemperature',20034, 0); %Only status code returned should be DRV_TEMP_OFF (since cooler is now off)
                ...
                if waitForWarmup
                    if ismember(obj.cameraType,{'ICCD'}) %ANDOR: What are 'Classic' cameras? 
                        ...
                        while currentTemperature < -20
                            currentTemperature = obj.apiCallFiltered('GetTemperature',20034, 0); %Only status code returned should be DRV_TEMP_OFF (since cooler is now off)
                        ...
                        while obj.apiCallFiltered('GetTemperature',20034, 0) < (startTemperature + 2)   %Only status code returned should be DRV_TEMP_OFF (since cooler is now off)
                            pause(.5);
                        end
    ...
    methods
        ...
        function val = get.statusString(obj)
            val = obj.apiResponseCodeMap(obj.status);
        end
        
        function val = get.temperatureStatus(obj)
            status = obj.apiCallRaw('GetTemperature',0);
            if ismember(status,obj.temperatureStatusCodes)
                val = obj.apiResponseCodeMap(status);
            else
                obj.apiProcessErrorResponseCode(status,'GetTemperature');
            end
        end
        
        function val = get.temperatureTarget(obj)
            %
            [~,val,~,~] = obj.apiCall('GetTemperatureStatus',0,0,0,0);
        end
        
                
        function val = get.versionInfoDriver(obj)            
            a = char(ones(512,1));
            val = obj.apiCall('GetVersionInfo','AT_SDKVersion',a,512); %TMW: This works sometimes, but is prone to seg faults
            %val = obj.apiCall('GetVersionInfo',1073741825,a,512);
        end
        
        function val = get.versionInfoFirmware(obj)
            a = char(ones(512,1));
            val = obj.apiCall('GetVersionInfo','AT_DeviceDriverVersion',a,512);            
            %val = obj.apiCall('GetVersionInfo',1073741824,a,512);
        end
        ...
        function set.HSSpeedIdx(obj, val)
            ...
            try 
                ...
                if isnumeric(val) && isscalar(val) && ismember(val,1:obj.maxNumberHSSpeeds)
                    ...
                    obj.apiCall('SetHSSpeed',obj.outputAmplifierMap(obj.outputAmplifier),val-1);                    
                    obj.HSSpeedIdx = val;
                else
                    error(errMsg);
                end
        ...
        function set.temperatureTarget(obj,val)
            ...
            obj.apiCall('SetTemperature',val);
    ...
    methods (Hidden)
        ...
        function val = getSimple(obj,propName)
            val = obj.apiCall(['Get' upper(propName(1)) propName(2:end)], 0);
        end
        
        function val = getSimpleString(obj,propName)
           val = obj.apiCall(['Get' upper(propName(1)) propName(2:end)], char(ones(1,255)));
        end
        
        function val = getSimpleBinary(obj,propName)
            sdkFuncName = [upper(propName(1)) propName(2:end)]; %Convert from isXXX to IsXXX
            val = logical(obj.apiCall(sdkFuncName,0));                                  
        end
        
        function val = getSimpleIndexed(obj,propName)
            val = obj.apiCall(['Get' upper(propName(1)) propName(2:end)], obj.(obj.indexMap(propName))-1, 0); %Switch from our 1-based to API 0-based indexing
        end
        ...
        function val = getSimpleMultiple(obj,propName)
            ...
            numOutputArgs = obj.apiFuncNargoutMap(funcName);
            dummyArgs = num2cell(zeros(numOutputArgs,1));
            
            val = cell(1,numOutputArgs);
            [val{:}] = obj.apiCall(funcName, dummyArgs{:});
            
            %val = {obj.apiCall(funcName, dummyArgs{:})}; % Seems to work too!
        end
        
        
        function val = getTemperatureValue(obj,propName)
            propName(1) = upper(propName(1));
            val = obj.apiCallFiltered(['Get' propName],obj.temperatureStatusCodes, 0);
        end
        ...
        function val = getAmpDesc(obj)
            dummyString = char(ones(1,21));
            val = obj.apiCall('GetAmpDesc',obj.outputAmplifierMap(obj.outputAmplifier),dummyString,length(dummyString));
        end
        
        function val = getAmpMaxSpeed(obj)
            val = obj.apiCall('GetAmpMaxSpeed',obj.outputAmplifierMap(obj.outputAmplifier),0);
        end
        
        function val = getControllerCardModel(obj)
            dummyString = char(ones(1,10));
            val = obj.apiCall('GetControllerCardModel',dummyString);
        end
        ...
        function val = getFKVShiftSpeed(obj)
            val =  obj.apiCall('GetFKVShiftSpeedF',obj.FKVShiftSpeedIdx-1,0);
        end
        ...
        function val = getMaximumBinning(obj)
            maximumBinningHorizontal = obj.apiCall('GetMaximumBinning',obj.readModeIdx-1,0,0);
            maximumBinningVertical = obj.apiCall('GetMaximumBinning',obj.readModeIdx-1,1,0);
        ...
        function setSimple(obj,propName,val)
            obj.apiCall(['Set' upper(propName(1)) propName(2:end)], val);
        end
        ...
        function setSimpleIndexedString(obj,propName,val)
            ...
            obj.apiCall(['Set' upper(propName(1)) propName(2:end)],idx);
        end
        
        function setSimpleMultiple(obj,propName,vals)
            % Assume that caller passed in a cell array of vals
            obj.apiCall(['Set' upper(propName(1)) propName(2:end)], vals{:});
        end
        ...
        function setMultiTrack(obj, val)
            ...
            if length(val) == 3
                [bottom, gap] = obj.apiCall('SetMultiTrack',val{1},val{2},val{3},0,0);    
        ...
        function setOutputAmplifier(obj, val)
            ...
            obj.apiCall('SetOutputAmplifier',outputAmplifierIdx);           
        end
    ...
    methods (Access=private)        
        ...
        function setIndexedRaw(obj,propName,val)
            ...
            if isnumeric(val) && isscalar(val) && ismember(val,1:length(obj.([propName 'Options'])))
                obj.apiCall(['Set' upper(propName(1)) propName(2:end)],val-1);
            else
                error(errMsg);
            end
        end
    ...
    methods (Access=protected)
        ...
        %              warning(['Error while setting ' src.Name ': ' obj.mapReverseDecode(obj.apiResponseCodeMap,str2num(ME.message(startIdx:endIdx)))]);
        %          end
    end
    ...
    methods
        function display(obj)
            ...
            propList = sort({'cameraSerialNumber' 'cameraType' 'statusString' 'isCoolerOn' 'EMCCDGain' 'EMAdvanced' 'EMGainMode' 'frameTransferMode'  ...
            'acquisitionMode' 'readMode' 'triggerMode'...
            'accumulationCycleTime' 'exposureTime' 'kineticCycleTime'...
            'numberAccumulations' 'numberKinetics' 'numberPrescans' ...
            'image' 'imageFlip' 'imageRotate' ...
    ...
    methods
        ...
        function abortAcquisition(obj)
            obj.apiCallFiltered('AbortAcquisition',[20073]);
        end
        
        function cancelWait(obj)
           obj.apiCall('CancelWait'); 
        end
        ...
        function coolerOFF(obj)
            obj.apiCall('CoolerOFF');
        end
        
        function coolerON(obj)
            obj.apiCall('CoolerON');
        end
        
        function freeInternalMemory(obj)
            obj.apiCall('FreeInternalMemory');
        end
        ...
        function GPIBReceive(obj,id,address,text,size)
            ...
            obj.apiCall('GPIBReceive',id,address,text,size);
        end
        
        function GPIBSend(obj,id,address,text)
            %See Andor SDK documentation for description of this function and its arguments 
            
            obj.apiCall('GPIBSend',id,address,text);
        end
        ...
        function val = inAuxPort(obj,port)
           ...
           val = obj.apiCall('InAuxPort',port,0);
        end
        ...
        function val = isPreAmpGainAvailable(obj, preAmpGainIdx, ADChannel, outputAmplifier, HSSpeedIdx)
            ...
            val = obj.apiCall('IsPreAmpGainAvailable',ADChannel,outputAmplifier,HSSpeedIdx-1,preAmpGainIdx-1,0);
        ...
        function outAuxPort(obj,port,state)
            ...
            obj.apiCall('OutAuxPort',port,state);
        end
        
        function prepareAcquisition(obj)
            obj.apiCall('PrepareAcquisition');
        end
        ...
        function saveAsBmp(obj,path,palette,ymin,ymax)
            ...
            obj.apiCall('SaveAsBmp',path,comment);  
        end
        ...
        function saveAsCommentedSif(obj,path,comment)
            ...
            obj.apiCall('SaveAsCommentedSif',path,comment);  
        end
        ...
        function saveAsEDF(obj,path,imode)
            ...
            obj.apiCall('SaveAsEDF',path,imode);  
        end
        ...
        function saveAsFITS(obj,path,typ)
            ...
            obj.apiCall('SaveAsFITS',path,typ);  
        end
        
        function saveAsPC(obj,path)
            %See Andor SDK documentation for description of this function and its arguments                       
            
            obj.apiCall('SaveAsPC',path);  
        end
        ...
        function saveAsRaw(obj,path,typ)
            ...
            obj.apiCall('SaveAsRaw',path,typ);  
        end
        
        function saveAsSif(obj,path)
            %See Andor SDK documentation for description of this function and its arguments                       
            
            obj.apiCall('SaveAsSif',path);  
        end
        ...
        function saveAsTiff(obj,path,typ,position,palette)
            ...
            obj.apiCall('SaveAsTiff',path,palette,position,typ);  
        end
        ...
        function saveAsTiffEx(obj,path,typ,position,palette,mode)
            ...
            obj.apiCall('SaveAsTiffEx',path,palette,position,typ,mode);            
        end
        ...
        function sendSoftwareTrigger(obj)
            ...
            obj.apiCall('SendSoftwareTrigger');
        end
        
        function shutDown(obj)
            %See Andor SDK documentation for description of this function and its arguments
            
            obj.apiCall('ShutDown');
        end
        
        function startAcquisition(obj)
            %See Andor SDK documentation for description of this function and its arguments
            
            obj.apiCall('StartAcquisition');
        end
        
        function waitForAcquisition(obj)
            %See Andor SDK documentation for description of this function and its arguments
            
            obj.apiCall('WaitForAcquisition');
        end
        
        function waitForAcquisitionTimeOut(obj,val)
            %See Andor SDK documentation for description of this function and its arguments
            
            obj.apiCall('WaitForAcquisitionTimeOut',val);
        end
    ...
    methods (Hidden)
        function cameraTypeMap = zprvExtractCameraTypeMap(obj)            
            [~,cameraTypeMap] = obj.extractCodeMap('^#define AC_CAMERATYPE_(\w*)\s*(\d*)'); %Extracts from header file
        end
    end
    ...
    methods (Access=private)
        ...
        function initializeModelPropValues(obj)
            ...
            obj.apiCall('GetCapabilities',andorCapsStruct);
            
            cameraTypeMap = obj.accessAPIDataVar('cameraTypeMap');            
            obj.cameraType = cameraTypeMap(andorCapsStruct.ulCameraType);            
            ...
            numOutputAmplifiers = obj.apiCall('GetNumberAmp',0);
            obj.outputAmplifierMap = containers.Map({'dummy'},{1});
            obj.outputAmplifierMap.remove('dummy');
            for i=0:numOutputAmplifiers-1
                dummyString = char(ones(1,21)); %API specifically says max number is 21 characters..using longer results in seg violations                              
                
                numTries = 0;
                while numTries < 10
                    numTries = numTries + 1;
                    name = obj.apiCall('GetAmpDesc',i,dummyString,length(dummyString));
            ...
            numPreAmpGains = obj.apiCall('GetNumberPreAmpGains',0);
            obj.preAmpGainOptions = cell(numPreAmpGains,1);
            
            for m=1:numPreAmpGains   
                obj.preAmpGainOptionsAll{m} = obj.apiCall('GetPreAmpGain',m-1,0);
            end

            for i=1:numADChans
                for j=1:numOutputAmplifiers
                    %Determine number of speeds
                    numHSSpeeds = obj.apiCall('GetNumberHSSpeeds',i-1,j-1,0);
                    ...
                    for k=1:numHSSpeeds                        
                        speedValCell{k} = obj.apiCall('GetHSSpeed',i-1,j-1,k-1,0);
                        
                        preAmpGainCell{k} = false(numPreAmpGains,1); %Initialize logical array
                        for m=1:numPreAmpGains                                                        
                            preAmpGainCell{k}(m) = obj.apiCall('IsPreAmpGainAvailable',i-1,j-1,k-1,m-1,0);
                        end
                    end
            ...
            for i=1:numVSSpeeds
                obj.VSSpeedOptions{i} = obj.apiCall('GetVSSpeed',i-1,0);
            end
            ...
            for i=1:numFKVShiftSpeeds
                obj.FKVShiftSpeedOptions{i} = obj.apiCall('GetFKVShiftSpeed',i-1,0);
            end
    ...
    methods (Hidden)
        ...
        %TODO: Figure out if better way to avoid duplication between apiCall/apiCallRaw
        ...
        %                 try
        %                     obj(i).apiProcessErrorResponseCode(status,funcName,obj(i).apiFilteredResponseCodes);
        %                 catch ME
        %                     ME.throwAsCaller();
        %                 end
        %             end
        ...
        %                 %Throw error if status code represent an error
        %                 try
        %                     obj(i).apiProcessErrorResponseCode(status,funcName,ignoredCodes);
        %                 catch ME
        %                     ME.throwAsCaller();
        %                 end
        %             end
    ...
    methods (Access=private)
        ...
        %                 statusString = obj.apiResponseCodeMap(status);
        %
        ...
        function val = isXXXAvailableHelper(obj, xxxFuncName, xxxPropName, xxxUnavailableResponseCode, xxxSuppliedVal)
            ...
            if strcmpi(obj.apiResponseCodeMap(status),xxxUnavailableResponseCode)
                val = false;
            else
                try
                    obj.apiProcessErrorResponseCode(status); %Throw any not 'expected' error
                catch ME
                    ME.throwAsCaller();
                end
                val = true;
