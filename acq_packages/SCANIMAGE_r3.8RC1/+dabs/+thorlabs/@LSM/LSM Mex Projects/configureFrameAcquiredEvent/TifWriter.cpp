...
/*
  ...
  * Overall structure of TIFWriter TIFF file. Both header and frames
    have tail-padding, or tail-nonesense bytes. The header is as
...
void TifWriter::closeTifFile(void) {
  if (fTiffFH!=NULL) {
    ...
    if (upos!=TifWriter::FIRSTIFDFILEOFFSET) {
      ...
      int ecode = fseek(fTiffFH,x,SEEK_CUR);
      if (ecode!=0) {
...
void TifWriter::setupIFD(void) {
  ...
  fSuppIFD = (char*)mxCalloc(fSuppIFDSize,sizeof(char));
  if (fSuppIFD==NULL) {
    handleErr("Problem allocating supplementary IFD.");
  }
  mexMakeMemoryPersistent(fSuppIFD);
...
//   //errHandler(TIFFSetField(fTiffFH, TIFFTAG_STRIPBYTECOUNT, xxxrps)); 
...
//   }
//   TIFFFlush(fTiffFH); //xxx
// }


// // xxx sizeof(char) must equal sizeof(tdata_t) which is sizeof(void*)
