    ...
    methods
        ...
        function delete(obj)
            if ~obj.cancelConstruct
                ...
                obj.stop(); %This will give error if a finite acquisition is runnin
        ...
        function stop(obj)
            ...
            for i=1:length(obj)
                obj(i).apiCall('DAQmxStopTask', obj(i).taskID);
            end
        end
        
        function abort(obj)
            %Identical to stop(), but should be used to abort ongoing Finite acquisitions/generations.
            %Unlike stop(), DAQmx Warning 200010 is suppressed. This warning occurs when stop() is called to terminate a Finite acquisition/generation prior to the specified number of samples
            %VECTORIZED
            
            for i=1:length(obj)
                obj(i).apiCallFiltered('DAQmxStopTask',200010, obj(i).taskID);
            end
        end
        ...
        function tf = isTaskDoneQuiet(obj)
            ...
            tf = false(size(obj));
            for c = 1:numel(obj)
                tf(c) = obj(c).apiCallFiltered('DAQmxIsTaskDone',200010, obj(c).taskID,0);
            end
        end
        ...
        function [outputData, sampsPerChanRead] =  readCounterData(obj,numSampsPerChan, timeout)
            
            %Process input arguments
            error(nargchk(1,3,nargin,'struct'));
            
            if nargin < 2 || isempty(numSampsPerChan)
                numSampsPerChan = inf;
            end
            
            if nargin < 3 || isempty(timeout) || isinf(timeout) || timeout < 1
                timeout = -1; 
            end            
            ...
            if strcmpi(get(obj.channels(1),'measType'),'DAQmx_Val_CountEdges')
                outputData = zeros(numSampsPerChan,1,'uint32');
                [outputData, sampsPerChanRead] = double(obj.apiCall('DAQmxReadCounterU32',obj.taskID,numSampsPerChan,timeout,outputData,numSampsPerChan,0,libpointer()));                
            else
                outputData = zeros(numSampsPerChan,1);
                [outputData, sampsPerChanRead] = obj.apiCall('DAQmxReadCounterF64',obj.taskID,numSampsPerChan,timeout,outputData,numSampsPerChan,0,libpointer());                
            end            
