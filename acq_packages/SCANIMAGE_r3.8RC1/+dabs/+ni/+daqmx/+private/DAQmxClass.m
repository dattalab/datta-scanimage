...
%     end
    ...
    properties (Constant, Hidden)
        ...
        apiSupportedVersionNames = {'8.8' '9.3.x' '9.4.x'}; %A list of shorthand names for API versions supported by this wrapper class
        ...
        apiHeaderFilenames = 'NIDAQmx_proto.m'; %Either a single name of the header filename (with the '.h' extension - OR a .m or .p extension), or a Map of such names keyed by values in 'apiSupportedVersionNames'
        
        apiCachedDataPath = [fileparts(which('dabs.ni.daqmx.private.DAQmxClass')) filesep 'private'];
    end
    ...
    methods (Hidden)
        function codeMapStruct = extractDAQmxCodeMaps(obj)
            ...
            [codeNameMap, codeValueMap] = obj.extractCodeMap('#define\s*(DAQmx_Val_\w*)\s*([^/]*)', fullfile(fileparts(fileparts(obj.apiHeaderFileName)),'NIDAQmx.h'));
