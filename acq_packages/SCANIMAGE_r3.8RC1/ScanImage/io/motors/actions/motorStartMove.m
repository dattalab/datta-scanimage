...
%
%% SYNTAX
%   newPosn: <OPTIONAL> Position, given as 3 element array, to which to start a move operation. If omitted/empty, the current state.motor.abs<X,Y,Z>Position or state.motor.rel<X,Y,Z>Position value will be used.
...
switch lower(coordType)
    case 'absolute'
        moveStartFcn = @hMotor.moveStartAbsolute;
        moveStartFcnZ = @hMotorZ.moveStartAbsolute;
    case 'relative'
        moveStartFcn = @hMotor.moveStartRelative;
        moveStartFcnZ = @hMotorZ.moveStartAbsolute;
    otherwise
...
if ~state.motor.motorZEnable
    ...
    if state.motor.dimensionsXYZZ %XYZ-Z
        moveStartFcn(newPosn(1:3));
