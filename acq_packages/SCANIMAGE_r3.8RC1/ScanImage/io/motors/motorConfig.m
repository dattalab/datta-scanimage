...
state.motor.excludeControls = [state.motor.excludeControls cellfun(@(x)gh.motorControls.(x),disableControlTags)];
...
if ~isempty(timeoutVal)
    ...
    hLSC.moveTimeout = timeoutVal; %TODO: Clarify in INI file that this is a MOVE timeout, not timeout for standard serial port command/reply pairs
end
