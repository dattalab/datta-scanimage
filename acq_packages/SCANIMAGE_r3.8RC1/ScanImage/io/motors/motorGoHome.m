...
if cycleHome && state.cycle.cycleOn && state.cycle.returnHomeAtCycleEnd %Go-to cycle home
...
elseif cycleAbort && state.cycle.cycleOn && ~isempty(state.internal.iterationHomePosn) %Go-to iteration home
    gotoHome(state.internal.iterationHomePosn,setPosnMode);

elseif ~isempty(state.internal.initialMotorPosition) && state.acq.returnHome %Go-to stack home %VI: Seems that idea is that state.acq.returnHome is a global setting, not per configuration/cycle-iteration. Should think about/revisit. -- 05/18/2011    
    gotoHome(state.internal.initialMotorPosition,setPosnMode);
end
...
if any(abs(state.motor.lastPositionRead - homePosn) > state.motor.hMotor.resolutionBest)
    doMove = true;
elseif numel(state.motor.lastPositionRead) > 3
