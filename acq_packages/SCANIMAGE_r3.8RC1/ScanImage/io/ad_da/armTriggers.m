...
if state.acq.externallyTriggered && ~forceInternal
    if state.acq.pureNextTriggerMode
        terminal = state.acq.nextTrigInputTerminal;
        ...
        terminal = state.acq.startTrigInputTerminal;
...
if ~isempty(state.acq.nextTrigInputTerminal) && nextEvent
    state.init.hNextTrigCtr.set('sampClkSrc', state.acq.nextTrigInputTerminal, 'sampClkActiveEdge', trigEdgeEncode(state.acq.nextTrigEdge));
