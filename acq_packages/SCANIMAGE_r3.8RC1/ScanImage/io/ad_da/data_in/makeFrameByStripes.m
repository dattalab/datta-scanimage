...
% getTime = toc;
    ...
    if grabStop
        ...
        if state.acq.numberOfZSlices > 1 && state.internal.zSliceCounter < (state.acq.numberOfZSlices - 1) %VI080911A
...
if ~isinf(state.acq.framesPerFile) && frameCount == state.acq.framesPerFile  && (state.acq.framesPerFile == 1 || mod(frameCount + 1,state.acq.framesPerFile)==1) 
...
if ~state.motor.motorZEnable || ~state.motor.dimensionsXYZZ
    ...
    if state.motor.motorZEnable %either xy-z mode or xyz-z mode with motorZEnable switched on
        %moveSecZ = true;
    ...
    else %xyz-z mode with motorZEnable switched off
        state.motor.absZPosition = state.internal.initialMotorPosition(3) - state.acq.stackCenteredOffset + state.acq.zStepSize * (state.internal.zSliceCounter + 1);
        state.motor.absZZPosition = state.internal.initialMotorPosition(4);
    end    
end
