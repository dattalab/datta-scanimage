...
% Close GUIs (which default to open)
guiNamesToClose = {'clockExportGUI' 'uncagingMapper' 'laserFunctionPanel' 'powerBox' 'fastConfigurationGUI' 'userFunctionsGUI' ...
                    'cycleGUI' 'metaStackGUI' 'roiGUI' 'roiDisplayGUI' 'positionGUI'};
for i = 1:length(guiNamesToClose)
    set(gh.(guiNamesToClose{i}).figure1,'Visible','Off');
end
...
state.motor.excludeControls = cellfun(@(x)gh.motorControls.(x),hideControls);
