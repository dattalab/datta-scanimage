...
if ~isempty(eventdata.NewData)
	...
	if valid
		...
		state.hSI.zprvError('Invalid cycle data.',['Invalid Cycle data entry: ' message]);
...
if regexp(val,'^\[.*\]$')
	...
	if ~isempty(tokens)
		...
		if length(val) < padLength
            padData = repmat(padVal,1,padLength-1);
            if padEnd
                val = [val padData];
            else
                val = [padData val];
		...
		elseif length(val) > padLength
			outputString = '';
			state.hSI.zprvDisp('Invalid input string.');
			return;
		end
		outputString = ['[' num2str(val) ']'];
		return;
	end
elseif isscalar(str2double(val)) == 1
