...
if ~isempty(getCycleVar('motorActionID'))
    ...
    switch motorActionType
        ...
        case 'ROI #'
            state.hSI.roiGotoROI(actionID,false,true);
        case 'Step'
            stepPos = actionID;
...
% handle power
val = getCycleVar('power');
if ~isempty(val)
    if length(val) > state.init.eom.numberOfBeams
        ...
        for i=1:length(val)
            ...
            state.init.eom.maxPower(i) = val(i);
            updateMaxPowerDisplay(i,val(i));
        end
    end
end
