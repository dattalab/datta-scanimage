...
if forceSquare
    % determine image size, depending on where user is drawing
    if userData.getRectFromAxes.ax == state.hSI.hROIDisplayAx
        sizeImage = [diff(get(userData.getRectFromAxes.ax,'XLim')) diff(get(userData.getRectFromAxes.ax,'YLim'))];
    else
        sizeImage = [state.acq.pixelsPerLine state.acq.linesPerFrame];
    end

    % normalize the coords, relative to the size of the Axes
    widthNorm = (width/sizeImage(1));
    heightNorm = (height/sizeImage(2));

    % if the user is drawing in an acquisition window, take SAM into account.
    if userData.getRectFromAxes.ax ~= state.hSI.hROIDisplayAx
        if state.acq.scanAngleMultiplierFast > state.acq.scanAngleMultiplierSlow
            samRatio = state.acq.scanAngleMultiplierSlow/state.acq.scanAngleMultiplierFast;

            widthNorm = heightNorm*samRatio;
        elseif state.acq.scanAngleMultiplierFast < state.acq.scanAngleMultiplierSlow
            samRatio = state.acq.scanAngleMultiplierFast/state.acq.scanAngleMultiplierSlow;

            heightNorm = widthNorm*samRatio;
        else
            widthNorm = min(widthNorm,heightNorm);
            heightNorm = widthNorm;
        end
    else
        widthNorm = min(widthNorm,heightNorm);
        heightNorm = widthNorm;
    ...
    width = widthNorm*sizeImage(1);
    height = heightNorm*sizeImage(2);
end
