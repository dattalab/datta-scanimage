% photodiode/get - Query internal fields of the object.
%
% SYNTAX
%  PROPERTIES = get(photodiode) - Gets all the fields in a photodiode object.
%  PROPERTY = get(photodiode, NAME) - Gets the value of the NAME field in a photodiode object.
%  PROPERTIES = get(photodiode, NAME, ...) - Gets the value of each named field in a photodiode object.
...
if isempty(varargin)
    varargout{1} = photodiodeObjects(this.ptr);
    return;
end
...
for i = 1 : length(recognized)
    varargout{i} = photodiodeObjects(this.ptr).(recognized{i});
end
