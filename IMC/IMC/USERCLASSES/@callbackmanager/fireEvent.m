...
%   VI060908A: Based on callbackArgs property, pass on callback-specific arguments to the callback -- Vijay Iyer 6/9/08
%   TO073008E: Removed VI060908A because it was redundant and ugly. Arguments were always supported by @callbackManager. Clearly, he didn't even glance at the code before changing it. -- Tim O'Connor 7/30/08
    ...
    try
        switch lower(class(callbacks(order(i)).callbackSpec))
            ...
            case 'function_handle'
                feval(callbacks(order(i)).callbackSpec, varargin{:}); %Added callbackArgs (VI060908A) - TO073008E - Had you bothered to even look at the code you were modifying, you'd see it already supported callbacks.
            case 'cell'
                feval(callbacks(order(i)).callbackSpec{:}, varargin{:}); %Added callbackArgs (VI060908A) - TO073008E
            otherwise
