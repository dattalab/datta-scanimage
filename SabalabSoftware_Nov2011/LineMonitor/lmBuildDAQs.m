function lmBuildDAQs
	...
	while isfield(state.lm, ['paramName' num2str(counter)])
		if (getfield(state.lm, ['active' num2str(counter)])==1) && ...
			...
			if (devSelection>0) && (devSelection<=length(state.lm.deviceIDs))
				...
				devList(end+1)=str2num(devSelection(4:5)); 
			else
	...
	for devCounter=1:length(minDevList)
		devNumber=minDevList(devCounter);
		try
			ad=[];
			state.lm.devices(devCounter).deviceHandle=analoginput('nidaq',['Dev' num2str(devNumber)]);
			ind=find(devList==minDevList(devCounter));
			...
			disp(['LineMonitor: Failed to create DAQ for device: Dev' num2str(devNumber) ' line: ' adTxt]);
		end
