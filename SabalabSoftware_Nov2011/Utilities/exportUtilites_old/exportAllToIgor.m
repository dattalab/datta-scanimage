...
% exportAllToIgor(prefix, template, clearFromMemory, forceDisk, avgMode, sizeLimit)
%
...
%
    tic
        if nargin<6
            sizeLimit=2000;		% don't try to retreive and export any "waves" greater than 500 Kb
        end
        if nargin<5
            forceLoadFromDisk=1;
        end
        ...
        for counter=1:length(files)	%loop through the filelist
            if ~files(counter).isdir	% made sure its a file and not a directory
                [path, fname, fext, ver]=fileparts(files(counter).name);
